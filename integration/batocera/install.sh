...
require_source_file "$SOURCE_DIR/bin/r4-ecctl"
require_source_file "$SOURCE_DIR/bin/r4-led-state"
require_source_file "$SOURCE_DIR/services/R4Controller"
require_source_file "$SOURCE_DIR/scripts/R4GameState"
require_source_file \
...
mkdir -p "$R4_DIR" || fail "Unable to create $R4_DIR"
mkdir -p "$SERVICE_DIR" || fail "Unable to create $SERVICE_DIR"
mkdir -p "$SCRIPT_DIR" || fail "Unable to create $SCRIPT_DIR"
mkdir -p "$ACHIEVEMENT_DIR" ||
    fail "Unable to create $ACHIEVEMENT_DIR"

cp "$SOURCE_DIR/bin/r4-ecctl" \
    "$R4_DIR/r4-ecctl" ||
    fail "Unable to install r4-ecctl"

cp "$SOURCE_DIR/bin/r4-led-state" \
    "$R4_DIR/r4-led-state" ||
    fail "Unable to install r4-led-state"

cp "$SOURCE_DIR/services/R4Controller" \
    "$SERVICE_DIR/R4Controller" ||
    fail "Unable to install R4Controller service"

cp "$SOURCE_DIR/scripts/R4GameState" \
    "$SCRIPT_DIR/R4GameState" ||
    fail "Unable to install R4GameState"

cp "$SOURCE_DIR/emulationstation/achievements/R4Achievement" \
...
if [ "$firmware_version" = "$EXPECTED_VERSION" ]; then
    ...
    echo "WARNING: expected '$EXPECTED_VERSION'" >&2

    if [ -n "$firmware_version" ]; then
        echo "WARNING: connected controller reports '$firmware_version'" >&2
    else
        echo "WARNING: embedded controller is unavailable" >&2
    fi
fi

echo
"$SERVICE_DIR/R4Controller" status || true
